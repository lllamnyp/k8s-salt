...
{% set etcdV = salt['pillar.get']('etcd:version') %}
get_etcd_archive:
  file.managed:
    - name: /data/etcd/etcd-{{ etcdV }}.tar.gz
    - source: https://github.com/etcd-io/etcd/releases/download/{{ etcdV }}/etcd-{{ etcdV }}-linux-amd64.tar.gz
    - skip_verify: true
    - user: root
    - group: devops
    - mode: 644
...
place_etcd_binaries:
  file.managed:
    ...
    - names:
      - /usr/local/bin/etcd:
        - source: /data/etcd/{{ etcdV }}/etcd-{{ etcdV }}-linux-amd64/etcd
      - /usr/local/bin/etcdctl:
        - source: /data/etcd/{{ etcdV }}/etcd-{{ etcdV }}-linux-amd64/etcdctl
    - require:
      - unpack_etcd_archive
  ...
  x509.certificate_managed:
    ...
    - names:
      - /etc/kubernetes/pki/apiserver.pem:
        ...
        - subjectAltName: "DNS:localhost, DNS:kubernetes, DNS:kubernetes.default, DNS:kubernetes.default.svc, DNS:kubernetes.default.svc.cluster.local, IP Address:{{salt['grains.get']('fqdn_ip4')[0]}}, IP Address:10.43.0.1, IP Address:127.0.0.1"
